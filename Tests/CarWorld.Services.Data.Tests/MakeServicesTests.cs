...
namespace CarWorld.Services.Data.Tests
{
    public class MakeServicesTests
    {
        ...
        [Test]
        public async Task IsMakeExistingByIdAsyncShouldReturnTrue()
        {
            await this.MakesSeedingAsync(5);

            var isTrue = await this.makesService.IsMakeExistingByIdAsync(1);

            Assert.IsTrue(isTrue);
        }

        [Test]
        public async Task IsMakeExistingByIdAsyncShouldReturnFalse()
        {
            await this.MakesSeedingAsync(5);

            var isFalse = await this.makesService.IsMakeExistingByIdAsync(6);
